...
function CustomToast({loading}: {loading: boolean}) {
    ...
    // Exibe o toast automaticamente quando "loading" muda para true
    useEffect(() => {
        if (loading) {
            setOpen(true)
            reset()
    ...
    return (
        <ToastProvider swipeDirection="left" duration={toastDuration + 300}>
            <Toast open={open} onOpenChange={handleOpenChange}>
                <div className="flex w-full justify-between gap-3">
                    <CircleCheckIcon className="mt-0.5 shrink-0 text-emerald-500" size={16} aria-hidden="true" />
                    <div className="flex grow flex-col gap-3">
                        <div className="space-y-1">
                            <ToastTitle>Loading projects!</ToastTitle>
                        </div>
                    </div>
                    <ToastClose asChild>
                        <button className="group -my-1.5 -me-2 size-8 shrink-0 p-0 hover:bg-transparent" aria-label="Close notification">
...
export {CustomToast, ButtonToast}
